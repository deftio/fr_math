...
* Fixed-radix multiply/add/sub with saturation and round-to-nearest
* Division and modulus (`FR_DIV`, `FR_MOD`) with 64-bit pre-scaling
* Utility macros: `FR_MIN`, `FR_MAX`, `FR_CLAMP`
* Trigonometric operations: sin, cos, tan and inverses (asin, acos, atan, atan2)
* Quick interpolation
* Logarithms: log2, ln, log10 with mixed input/output radix
* Exponentials: pow2, exp, pow10 (high-accuracy and shift-only variants)
* Floor/ceil/truncate (in fixed radix)
* Square root and hypotenuse (round-to-nearest, ≤ 0.5 LSB error)
* Fast approximate magnitude (shift-only, no multiply, ~0.14% error)
* Wave generators: square, PWM, triangle, sawtooth, morphable triangle, LFSR noise
* ADSR envelope generator (linear, sample-rate independent)
* Conversions between degrees/radians/BAM without loss of precision
...
* No dependencies — compiles on all C99 targets with no floating-point libraries or emulation needed
...
functionality. v2.0.1 highlights: s15.16 trig output with exact cardinal
angles, inverse trig returns radians, round-to-nearest multiply/sqrt,
expanded pow2 table (65 entries), radix-28 constant multiply (`FR_MULK28`),
and shift-only exp/pow10 fast-path variants for 8-bit targets.
